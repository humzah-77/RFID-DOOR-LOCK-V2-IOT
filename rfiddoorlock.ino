 ...
 * This program is an arduino based rfid card reader door lock V2.
 * --------------------------------------------------------------------------------------------------------------------
 ...
 * with a simple touch, This version is now connected to the arduino cloud by connecting to an arduino nano33 iot
 * this allows the user to have a mastercard through the app 
 * 
 * 
 * Pin layout used:
 * -------------------------------------
...
void setup() { 
  ...
  pinMode(3, INPUT);//Main arduino opens door
  pinMode(4, OUTPUT);//tells main arduino if door is opened
  pinMode(14, INPUT);//main arduino sets board to adding mode
  pinMode(15, INPUT);//main arduino sets board to adding mode
  pinMode(lock, OUTPUT);
...
void loop() {
  ...
  if(digitalRead(3) == HIGH)//if master board sends high open door
  {
    opendoor();
  }
  if(digitalRead(14) == HIGH)//if master board sends high put into adding mode
  {
    nuidPICC[0] = master1;//put into master card mode 
    nuidPICC[1] = master2;
    nuidPICC[2] = master3;
    nuidPICC[3] = master4;
    sub = false;// put into adding mode
  }
  if(digitalRead(15) == HIGH)//if masterboard sends hight put in removing mode
  {
    nuidPICC[0] = master1;//put into master card mode 
    nuidPICC[1] = master2;
    nuidPICC[2] = master3;
    nuidPICC[3] = master4;
    sub = true;//puts arduino into subtracting mode 
  }
